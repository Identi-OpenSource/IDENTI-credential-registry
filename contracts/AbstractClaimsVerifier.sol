...
contract AbstractClaimsVerifier {
    ...
    function _registerCredential(address _issuer, address _subject, bytes32 _credentialHash, uint256 _from, uint256 _exp) internal returns (bool){
        return registry.register(_issuer, _subject, _credentialHash, _from, _exp);
    }
    ...
    function _verifyIssuer(bytes32 digest, address issuer, uint8 v, bytes32 r, bytes32 s) internal view returns (bool) {
        return registry.validTrustee(issuer, ecrecover(digest, v, r, s));
    }
    ...
    function _isActive(bytes32 digest, address issuer) internal view returns (bool){
        return registry.isActive(issuer, digest);
    }
